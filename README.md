...
**AI 시대, 당신은 어떤 일꾼 동물입니까?** 10문항, 1분. 결과는 다섯 동물 중 하나. AX 는 AI Transformation(AI 전환)의 국내 통용 약어다.

| 코드 | 타입 | 별명 | 한 줄 |
|---|---|---|---|
| PRTO | 🐱 **프로토타이퍼** Prototyper | 고양이 | 먼저 건드려 보는 고양이 |
| BLDR | 🐶 **빌더** Builder | 강아지 | 끝까지 물어오는 강아지 |
| SWPR | 🐰 **스위퍼** Sweeper | 토끼 | 군더더기를 쓸어내는 토끼 |
| GRWR | 🐻 **그로워** Grower | 곰 | 돌봐서 키우는 곰 |
| MNTR | 🦉 **메인테이너** Maintainer | 부엉이 | 밤새 지켜보는 부엉이 |

결과 코드는 `메인-서브첫글자` 꼴이다. `PRTO-G` 는 고양이면서 곰 기질이 섞인 사람.
