...
export const SITE = {
  ...
  length: '15문항, 2분',
  description: '고양이·강아지·토끼·곰·부엉이. Boris Cherny가 말한 5가지 아키타입을 동물로. 15문항, 2분.',
...
// 15문항 × 3지선다. 선택지 하나는 타입 하나를 가리키고 그 타입에 1점.
// 열다섯 문항 모두 "이 상황에서 먼저 무엇을 하나" 를 묻는다. 선호나 자기 이미지를 묻지 않는다.
// 균형 규칙: 45개 선택지에서 모든 타입이 정확히 9번. 한 문항 안에 같은 타입이 두 번 나오지 않는다.
// 짝 규칙: 열 가지 타입 조합이 4~5번씩 맞붙는다 — 특정 두 타입만 자주 비교되면 그 둘의 구분만 세밀해진다.
// (scripts/check.mjs 가 둘 다 검증)
export const QUESTIONS = [
  ...
  {
    text: '새 기능 제안서가 올라왔다. 검토 회의에 들어가기 전에 하는 일은?',
    options: [
      { text: '제안대로 동작하는 화면 하나를 만들어 놓고 들어간다', type: 'prototyper' },
      { text: '언제까지 무엇을 내놓을 수 있는지 범위와 날짜를 적어 간다', type: 'builder' },
      { text: '빼도 되는 항목을 표시해 분량을 절반으로 줄여 간다', type: 'sweeper' },
    ],
  },
  {
    text: '갑자기 반나절이 비었다. 그 시간에 무엇을 하나?',
    options: [
      { text: '전부터 궁금했던 새 도구로 작은 것을 하나 만들어 본다', type: 'prototyper' },
      { text: '지난주 사용 기록을 처음부터 끝까지 따라가며 어디서 막히는지 본다', type: 'grower' },
      { text: '미뤄둔 경고 알림과 오래된 의존성을 정리한다', type: 'maintainer' },
    ],
  },
  {
    text: '새 팀에 합류한 첫 주. 가장 먼저 손대는 것은?',
    options: [
      { text: '문서보다 먼저 화면과 코드를 이리저리 눌러 보며 감을 잡는다', type: 'prototyper' },
      { text: '유입부터 이탈까지 숫자를 훑어 어디가 새는지 찾는다', type: 'grower' },
      { text: '장애 기록과 당번 문서를 읽고 무엇이 자주 터지는지 파악한다', type: 'maintainer' },
    ],
  },
  {
    text: '손대기 겁나는 오래된 화면에 새 요구가 들어왔다. 어떻게 시작하나?',
    options: [
      { text: '지금 구조 위에서 요구를 채우는 가장 빠른 경로를 잡고 날짜를 정한다', type: 'builder' },
      { text: '그 화면에서 이제 쓰이지 않는 부분을 먼저 걷어낸다', type: 'sweeper' },
      { text: '새 구조로 다시 짠 하루짜리 버전을 만들어 비교해 본다', type: 'prototyper' },
    ],
  },
  {
    text: '분기 계획 회의. 하고 싶은 일 목록이 서른 개다. 당신이 하는 일은?',
    options: [
      { text: '실제로 낼 수 있는 개수만 남기고 각각에 날짜를 붙인다', type: 'builder' },
      { text: '겹치거나 효과가 작은 항목을 지워 목록을 절반으로 만든다', type: 'sweeper' },
      { text: '각 항목이 어떤 숫자를 움직이는지 적고 그 순서로 세운다', type: 'grower' },
    ],
  },
  {
    text: '새벽에 알림이 울렸고, 원인은 지난주 급하게 넣은 코드였다. 다음 날 아침에 하는 일은?',
    options: [
      { text: '같은 요구를 제대로 처리하는 버전을 이번 주 안에 내놓는다', type: 'builder' },
      { text: '급하게 넣은 그 경로를 통째로 걷어내고 원래 흐름으로 되돌린다', type: 'sweeper' },
      { text: '같은 일이 또 생기면 먼저 알 수 있게 감시와 복구를 붙인다', type: 'maintainer' },
    ],
  },
];
