...
실제 브라우저로 처음 오는 사람처럼 돌려 봤다 (320px·390px·1440px, 뒤로가기·앞으로가기·새로고침·다크 모드·키보드).

- **뒤로가기가 사이트를 벗어났다.** 문항 화면이 랜딩과 같은 주소였다. 모바일에서 뒤로 쓸어넘기는 건 흔한 동작인데 답이 전부 사라졌다 → 해시 라우팅
- **결과를 새로고침하면 결과가 사라졌다.** 결과도 같은 주소였다. 앱 전환 후 복귀·화면 회전·주소 복사가 전부 무의미했다 → `#r<답안>`
- `#q0` 같은 주소가 문항 번호를 -1 로 만들어 빈 화면이 될 수 있었다 → 1~10 으로 클램프
- 320px 에서 가로 스크롤 없음, 시작 버튼이 첫 화면에 보임, 선택지는 `<button>` 이라 키보드로 닿는다 — 이상 없음
- 다크 모드 대응은 없다 (라이트 한 벌로 의도한 것). 지금은 두지만, 밤에 보는 사람이 많으면 다시 본다
- 아트를 동물로 바꾼 뒤 문구도 같이 바꿨다. 이미지만 동물이고 문구가 "~하는 사람"이면 캐릭터가 삽화로 떨어진다. `nick` 을 동물 이름으로 두고(고양이·강아지·토끼·곰·부엉이) 결과·공유·랜딩·메타를 그 동물의 동사(건드리다·물어오다·쓸어내다·물 주다·지키다)로 통일했다. 문항 열 개는 사람의 말로 남겼다 (`docs/DESIGN.md` 참고)
...
| Decision | Rationale |
|---|---|
| 화면을 해시 주소로 (`#q<번호>`, `#r<답안>`) | 사용자 관점 테스트에서 셋이 깨졌다: 모바일 뒤로 쓸기 = 사이트 이탈, 결과 새로고침 = 결과 소멸, 주소 복사 = 랜딩만 보임. 결과 해시에 답을 담아 저장 장치 없이 복원한다 |
...
| 채점 1점(선택지 하나 = 타입 하나), 동점 규칙은 `docs/DESIGN.md` 채점 절이 원본 | 결정적이고 설명 가능. 같은 답이면 항상 같은 결과. 대신 동점이 잦다 (같은 절 "남은 것") |
...
| 문항에 primary/secondary 두 타입 배점 유지 | 한 선택지가 두 타입을 가리키면 무엇을 재는지 흐려진다. 특히 스위퍼·메인테이너가 서로의 secondary 로 자주 들어가 구분력을 깎았다. 1점·1타입으로 바꾸고, 대신 늘어난 동점은 DESIGN 에 수치로 남겼다 |
| 문항까지 동물 말투로 | 문항은 답하는 사람의 하루다. 캐릭터를 연기하게 만들면 자기 보고가 아니라 역할극이 되고, 직군 중립·장면 기반 원칙이 깨진다 |
| 타입 이름을 동물로 교체 (프로토타이퍼 → 고양이) | 원문 아키타입과의 연결이 끊긴다. 이름은 남기고 별명·문구·이미지를 동물로 |
...
10문항 × 3지선다 = 30 슬롯. 5타입에 6번씩 정확히 배분하려면 문항을 쓴 뒤 표로 세어봐야 한다. `check.mjs` 가 이걸 자동으로 잡으니 문항을 바꿀 땐 반드시 돌린다.
