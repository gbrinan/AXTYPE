...
1. 처음 오는 사람처럼 실제 브라우저로 구동 (320·390·1440px, 뒤로가기·앞으로가기·새로고침, 다크 모드, 키보드, 공유받은 화면, 망가진 주소)
2. 치명적인 것 둘: 문항·결과가 랜딩과 같은 주소라 (1) 모바일 뒤로 쓸기가 사이트를 벗어나 답이 날아가고 (2) 결과 새로고침이 결과를 지웠다
3. 해시 라우팅 도입: 문항 `#q<번호>`, 결과 `#r<답안>`. 결과 해시가 답을 담아 새로고침·새 탭에서도 같은 결과. 문항 도중 답은 sessionStorage, 막히면 1번으로
4. `#q0` 이 문항 번호를 -1 로 만들던 것 클램프. 이전 버튼을 `history.back()` 으로 통일
5. 회귀 테스트 6종을 `smoke.mjs` 에 추가
...
| 뒤로가기 (브라우저) | 4번 문항에서 뒤로 | 3번 문항, 고른 답 유지 | 유지됨 | ✅ |
| 새로고침 (문항 도중) | 4번 문항에서 새로고침 | 그 자리 유지 | 유지됨 | ✅ |
...
| 15문항 균형 | 45 선택지 | 타입마다 9번 | 9/9/9/9/9 | ✅ |
| 15문항 짝 균형 | 열 가지 짝 | 4~6번씩 맞붙음 | 4~6 (grower·maintainer 만 6) | ✅ |
| 15문항 분포 | 14,348,907 조합 전수 | 어떤 타입도 12% 미만 아님 | 18.8%~20.8% | ✅ |
| 정면 대결 동점 규칙 | 같은 전수 | 안 풀리는 1위 동점 15% 이하 | 10.0% (규칙 없을 때 30.1%) | ✅ |
| 막대 퍼센트 합 | 무작위 2,000 답안 + 브라우저 | 정확히 100% | 100% | ✅ |
| 15문항 브라우저 한 바퀴 | `npm run smoke` | 결과·해시·되돌아가기·새로고침·공유·저장 | 통과 | ✅ |
...
| Question | Answer |
|---|---|
| 1. 현재 어느 단계인가? | Phase 5: Delivery — Phase 13 까지 끝나 `main` 에 합쳐짐. 남은 것은 25명 실측(사람)과 anandara.xyz 흡수 |
| 2. 다음에 할 일은? | 25명 실측(첫 번째 못) → Threads·카카오톡에 `https://axtype.vercel.app` 붙여 미리보기 확인, 홈 화면 설치 확인 → anandara.xyz `/type/` 로 흡수 (타입 → 공룡 원소 + 추천 등급) |
| 3. 목표는? | Threads 에서 퍼지는 2분짜리 AX 타입 테스트 |
