...
Threads 에서 퍼지는 2분짜리 "AI 시대 일꾼 타입" 테스트. 프로토타이퍼·빌더·스위퍼·그로워·메인테이너 중 나는 누구인지 알려주고, 결과를 한 번의 탭으로 올리게 한다. 정적 파일만으로 어디든 배포.

## Current Phase

🔄 Phase 5: Delivery (Phase 6~13 완료, `main` 에 모두 합쳐짐, Vercel 배포 연결됨. 남은 것은 25명 실측 — 사람 몫)
...
- [x] 판별 여유: 1위 동점 34.7% → Phase 13 에서 처리

### Phase 13: 판별력 보강 (15문항 + 정면 대결 동점 규칙) ✅

- [x] 문항 10 → 15 (45 슬롯 ÷ 5타입 = 9번씩). 새 문항 5개 모두 행동 장면
- [x] 10번 문항을 판단 기준 질문에서 행동 질문으로 다시 씀 (Phase 12 에서 남겨둔 것)
- [x] 동점 규칙에 **정면 대결** 추가: 총점이 같으면, 두 타입이 한 문항에 함께 놓였던 문항에서 더 고른 쪽 (Copeland)
- [x] 그 규칙에 근거가 생기도록 짝 균형 설계: 열 가지 짝이 4~6번씩 맞붙게. `check` 가 강제
- [x] 채점기를 무할당 방식으로 (전수 3^15 = 14,348,907 조합이 1.8초). `score()` 와 `check` 가 같은 함수 공유
- [x] 막대 퍼센트를 최대 잔여법으로 — 15는 100을 나누어떨어지지 않는다. 합이 항상 정확히 100%
- [x] `check` 에 게이트 추가: 짝 균형, 안 풀리는 1위 동점 상한 15%, 퍼센트 합 100%
- [x] `smoke` 를 문항 수에 의존하지 않게 (`QUESTIONS.length` 기준)
- [x] 측정: 안 풀리는 1위 동점 34.7% → 10.0%, 타입 분포 14.4~23.2% → 18.8~20.8%
- [ ] 남은 열 하나: 동점 두 타입만 놓고 한 문항 더 묻기. 25명 실측 뒤에 판단 (사람)
...
| 15문항 × 3지선다, 1선택지 1타입 1점 | 45 슬롯을 5타입에 9번씩 정수로 나눌 수 있고, 짝 균형(4~6)까지 맞출 여유가 나오는 가장 작은 수 |
